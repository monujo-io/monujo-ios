...
- (id)init: (void *) internal {
    ...
    if (self) {
        m_walletImpl = (Monero::Wallet *) internal;
...
{
    Monero::PendingTransaction * ptImpl = m_walletImpl->createTransaction(std::string([dst_addr UTF8String]),
...
- (void) disposeTransaction: (PendingTransaction *) t {
    m_walletImpl->disposeTransaction((Monero::PendingTransaction*)t.internal);
}
...
{
    return [NSString stringWithUTF8String:Monero::Wallet::genPaymentId().c_str()];
}
...
+ (NSString*) displayAmount: (UInt64) amount {
    return [NSString stringWithUTF8String: Monero::Wallet::displayAmount(amount).c_str()];
}

+ (UInt64) amountFromString: (NSString*) amount
{
    return Monero::Wallet::amountFromString(std::string([amount UTF8String]));
}
